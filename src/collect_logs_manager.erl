...
-export([start_link/0, stop/0, begin_collection/2, cancel_collection/0,
...
start_log_collection(Nodes, Upload, State) ->
    ...
    case AllSuccess of
        true ->
            ...
            State1 = State#state{children = #children{ pending = ChildIDs,
                                                       in_progress = []},
                                 cancelled = false,
        ...
        false ->
            ?log_error("begin_collection: Failed to successfully start all children: ~p~n", [Children]),
            {reply, {error, already_running}, State}
    end.
...
multi_begin_cancel(Slaves) ->
    ...
    [?_assertEqual(in_progress, Result1),
     ?_assertEqual({ok, in_progress}, WaitResult),
     ?_assertEqual(ok, Result2),
     ?_assertMatch(idle, proplists:get_value(status, Status))].
